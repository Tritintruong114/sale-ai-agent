...
// Xưng hô đúng + dùng thông tin khách đã cho — câu đủ chủ ngữ–vị ngữ, không hỏi lại.
const ADDRESS_RULE = `Xưng hô: mở đầu gọi khách là "anh/chị". NGAY KHI khách để lộ cách xưng (vd nhắn "giao anh…", "chị lấy…", "mình ở…") thì gọi đúng theo đó ("anh" / "chị" / "mình") và giữ NHẤT QUÁN suốt hội thoại, KHÔNG dùng lại "anh/chị" chung chung nữa.
Dùng thông tin khách đã cung cấp (khu vực, số lượng, ngày cần, món đã chọn…): xác nhận lại rồi đi tiếp, TUYỆT ĐỐI không hỏi lại thứ khách vừa nói. Mỗi lượt chỉ hỏi đúng MỘT thông tin còn thiếu. Viết câu đủ chủ ngữ–vị ngữ, rõ ràng (vd "Em giao về quận 10 trong hôm nay được ạ"), không trả lời cụt hay đảo lộn ngữ pháp.`;
